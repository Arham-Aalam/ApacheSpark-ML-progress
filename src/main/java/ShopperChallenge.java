...
public class ShopperChallenge {
    ...
    String exdataPath = "/media/arham/FUN AND MULTIMIDIA/hotwax/acquire-valued-shoppers-challenge/";
    public Dataset<Row> trainHistory,
                        offers,
                        sampleSubmission,
                        chain,
                        offer,
    ...
    public void init() {
        ...
        spark = SparkSession.builder().appName("Clustering_POC")
    ...
    private void loadData() {
        trainHistory = spark.read().format("csv").option("header", "true").option("inferSchema", "true").load(exdataPath + "trainHistory.csv.gz");
        //trainHistory.show(20, false);
        System.out.println("Training data count : " + trainHistory.count());

        //offers = spark.read().format("csv").option("header", "true").option("inferSchema", "true").load(exdataPath + "offers.csv.gz");
        //offers.show(20, false);

        //sampleSubmission = spark.read().format("csv").option("header", "true").option("inferSchema", "true").load(exdataPath + "sampleSubmission.csv.gz");
        //sampleSubmission.show(20, false);

        //Dataset<Row> transactions = spark.read().format("csv").option("header", "true").option("inferSchema", "true").load(exdataPath + "transactions.csv.gz");
    ...
    private void clusterChain() {
        ...
        vectorAssembler.setInputCols(new String[]{"chain"})
    ...
    private void clusterOffer() {
        ...
        VectorAssembler vectorAssembler = new VectorAssembler();
        vectorAssembler.setInputCols(new String[]{"offer"})
                .setOutputCol("features");
    ...
    private void clusterMarket() {
        ...
        VectorAssembler vectorAssembler = new VectorAssembler();
        vectorAssembler.setInputCols(new String[]{"market"})
                .setOutputCol("features");
    ...
    private void clusterTrips() {
        ...
        VectorAssembler vectorAssembler = new VectorAssembler();
        vectorAssembler.setInputCols(new String[]{"repeattrips"})
                .setOutputCol("features");
    ...
    private void clusterOfferDate() {
        offerdate = trainHistory.withColumn("newOfferDate", trainHistory.col("offerdate").cast("String")).select("id", "newOfferDate").na().drop();
        //offerdate.show(20);

        int k = 6;
        int iterations = 10;

        Dataset<Row> indexedOfferDate = new StringIndexer()
                .setInputCol("newOfferDate")
                .setOutputCol("newOfferDates")
                .fit(offerdate)
                .transform(offerdate);

        VectorAssembler vectorAssembler = new VectorAssembler();
        vectorAssembler.setInputCols(new String[]{"newOfferDates"})
                .setOutputCol("features");
    ...
    private void clusterAll() {
        int k = 6;
        int itr = 10;
        allFeatures = trainHistory
                .withColumn("newOfferDate", trainHistory.col("offerdate").cast("String"))
                .select("id", "chain", "offer", "market", "repeattrips", "newOfferDate")
                .na()
                .drop();
